...
namespace Badeend.ValueCollections.Tests.Reference
{
    ...
    public class ValueList_Tests_Insert
    {
        internal class Driver<T>
        {
            ...
            {
                ValueList<T> list = new ValueList<T>(items);
            ...
            {
                ValueList<T> list = new ValueList<T>(constructIEnumerable(itemsX));
                ...
                list = new ValueList<T>(constructIEnumerable(itemsX));
            ...
            {
                ValueList<T> list = new ValueList<T>(constructIEnumerable(items));
                int[] bad = new int[] { items.Length + 1, items.Length + 2, int.MaxValue, -1, -2, int.MinValue };
                for (int i = 0; i < bad.Length; i++)
                {
            ...
            {
                ValueList<T> list = new ValueList<T>(items);
                ValueList<T> range = useSlice ? list.Slice(index, count) : list.GetRange(index, count);
            ...
            {
                ValueList<T> list = new ValueList<T>(items);
                ValueList<T> range = list[index..(index + count)];
            ...
            {
                ValueList<T> list = new ValueList<T>(items);
                ValueList<T> range = useSlice ? list[index..(index + count)] : list.GetRange(index, count);
            ...
            {
                ValueList<T> list = new ValueList<T>(itemsX);
            ...
            {
                var list = new ValueList<T>(items);
        ...
        {
            ValueList<int> list = new ValueList<int>() { 1, 2, 3 };
            ValueList<int> expectedList = new ValueList<int> { 1, 2, 3, 2, 3, 4, 3, 4, 4 };

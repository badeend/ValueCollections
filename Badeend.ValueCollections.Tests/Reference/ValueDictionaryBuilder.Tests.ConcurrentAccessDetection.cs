...
namespace Badeend.ValueCollections.Tests.Reference
{
    ...
    {
        private async Task DictionaryConcurrentAccessDetection<TKey, TValue>(ValueDictionaryBuilder<TKey, TValue> dictionary, bool isValueType, object comparer, Action<ValueDictionaryBuilder<TKey, TValue>> add, Action<ValueDictionaryBuilder<TKey, TValue>> get, Action<ValueDictionaryBuilder<TKey, TValue>> remove, Action<ValueDictionaryBuilder<TKey, TValue>> removeOutParam)
        {
        ...
        public async Task DictionaryConcurrentAccessDetection_ValueTypeKey(Type comparerType)
        {
            ...
            ValueDictionaryBuilder<int, int> dic = comparerType == null ?
                new ValueDictionaryBuilder<int, int>() :
                new ValueDictionaryBuilder<int, int>((customComparer = (IEqualityComparer<int>)Activator.CreateInstance(comparerType)));
        ...
        public async Task DictionaryConcurrentAccessDetection_ReferenceTypeKey(Type comparerType)
        {
            ...
            ValueDictionaryBuilder<DummyRefType, DummyRefType> dic = comparerType == null ?
                new ValueDictionaryBuilder<DummyRefType, DummyRefType>((customComparer = EqualityComparer<DummyRefType>.Default)) :
                new ValueDictionaryBuilder<DummyRefType, DummyRefType>((customComparer = (IEqualityComparer<DummyRefType>)Activator.CreateInstance(comparerType)));
